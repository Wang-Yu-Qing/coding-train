...
class grid(object):
    ...
    def show(self, screen):
        ...
        # if self.checked:
        #     for i in range(self.x, self.x + self.w - 1):
        #         for j in range(self.y, self.y + self.w - 1):
    ...
    def get_an_unchecked_neighbour(self):
        try:
            ...
            neighbour = random.choice([{'position':p, 'neighbour':n} for p, n in neighbours.items() if not n.checked])
            return neighbour
        except IndexError:
            return None 
